-- ============================================================================
-- scripts/migracion_farmacias_consolidada.sql
--
-- Script maestro, idempotente, para el proyecto Supabase "Denta Stock"
-- (ref ejoebkbkplvunqesiilh). Reemplaza el intento original de
-- supabase/migrations/20260826100000_*.sql .. 20260826100700_*.sql.
--
-- POR QUÉ ESTE SCRIPT ES DISTINTO DEL ORIGINAL — LEE ESTO PRIMERO:
--
-- Las migraciones incrementales fallaban con "tipo_movimiento no existe" /
-- "rol_usuario no existe" porque el diagnóstico original (basado en
-- src/types/database.types.ts, marcado en su propio encabezado como
-- "generado a mano, nunca verificado contra el proyecto real") estaba
-- equivocado: ESTA BASE DE DATOS NO USA TIPOS ENUM NATIVOS DE POSTGRES.
-- 'usuarios.rol', 'movimientos.tipo' y 'ubicaciones.tipo' son columnas
-- TEXT con un CHECK constraint, no un `CREATE TYPE ... AS ENUM`. Verificado
-- por introspección directa (pg_constraint, information_schema.columns)
-- contra el proyecto real antes de escribir este script — no es una
-- suposición.
--
-- Además, la introspección reveló que esto es más grande que el módulo de
-- farmacias: 'movimientos' tiene Row Level Security HABILITADO pero CERO
-- políticas (bloqueo total por defecto), no existe ningún trigger en todo
-- el schema 'public' (el documentado 'trg_procesar_movimiento' que debía
-- mantener 'stock_ubicacion' nunca se creó), y 'fn_obtener_rol_actual()'
-- (referenciada en comentarios de src/api/auth.ts) tampoco existe. La
-- tabla 'movimientos' tiene 0 filas hoy — consistente con que ningún
-- movimiento se ha podido registrar nunca contra este proyecto. Este
-- script por lo tanto reconstruye esa base (trigger + función de rol +
-- políticas admin) ADEMÁS de las piezas específicas de farmacias, porque
-- unas dependen de las otras.
--
-- CÓMO SE APLICÓ: vía `supabase db query --linked`, ejecutado como un
-- único bloque (sin BEGIN/COMMIT manual — no hace falta: al no usar
-- `ALTER TYPE ... ADD VALUE`, no aplica la restricción de Postgres de
-- "no se puede usar un valor de enum en la misma transacción en que se
-- agregó", así que sí es seguro correr todo el script de una sola vez).
--
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'movimientos') THEN
    RAISE EXCEPTION 'La tabla public.movimientos no existe — este script asume el esquema base de DentaStock ya desplegado (productos, ubicaciones, usuarios, lotes, stock_ubicacion, movimientos). Verifica que estás conectado al proyecto correcto antes de continuar.';
  END IF;
...
--    bloque es solo defensivo por si se corre este script contra otro
--    entorno donde todavía no exista.
-- ----------------------------------------------------------------------
...
--    documentada en el código (no hay DDL original que copiar):
--
...
-- ----------------------------------------------------------------------
-- 8-9. Políticas RLS específicas de farmacias (módulo original) —
--    aditivas, no tocan nada de lo anterior.
-- ----------------------------------------------------------------------
...
--     dependientes) y sin filtro adicional de "activos" — confirmado
--     con el usuario: se mantiene igual a como el Dashboard/Reportes ya
--     la consumen hoy.
-- ----------------------------------------------------------------------
...
-- ============================================================================
-- FIN. Verificación sugerida después de correr esto (todas de solo lectura):
--
--   SELECT rol, count(*) FROM usuarios GROUP BY rol;
--   SELECT tipo, count(*) FROM movimientos GROUP BY tipo;
--   SELECT tablename, policyname, cmd FROM pg_policies WHERE schemaname='public'
--     AND tablename IN ('movimientos','stock_ubicacion','lotes') ORDER BY 1,2;
--   SELECT tgname FROM pg_trigger WHERE tgrelid = 'public.movimientos'::regclass
--     AND NOT tgisinternal;
--
-- Y una prueba funcional real: como ADMIN, registrar una entrada de
-- proveedor de un producto de prueba y confirmar que stock_ubicacion sube
-- exactamente esa cantidad — es la primera vez que esa ruta se ejercita
-- de extremo a extremo contra este proyecto.
-- ============================================================================
